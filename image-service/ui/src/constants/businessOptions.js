export const useCaseOptions = [
  { value: 'main_product_image', label: 'Main product image' },
  { value: 'product_gallery_image', label: 'Product gallery image' },
  { value: 'collection_banner', label: 'Collection banner' },
  { value: 'sale_promo', label: 'Sale promo' },
  { value: 'instagram_ad', label: 'Instagram ad' },
  { value: 'story_creative', label: 'Story creative' },
];

export const productCategoryOptions = [
  { value: 'general', label: 'General' },
  { value: 'fashion', label: 'Fashion' },
  { value: 'electronics', label: 'Electronics' },
  { value: 'home', label: 'Home & Living' },
  { value: 'beauty', label: 'Beauty & Wellness' },
  { value: 'food', label: 'Food & Beverage' },
  { value: 'jewelry', label: 'Jewelry' },
];

export const outputFormatOptions = [
  { value: 'product_image', label: 'Square product image' },
  { value: 'product_gallery', label: 'Gallery image' },
  { value: 'collection_banner', label: 'Collection banner' },
  { value: 'sale_promo', label: 'Sale promo creative' },
  { value: 'instagram_ad', label: 'Instagram post' },
  { value: 'story_creative', label: 'Story / reel' },
];

export const generationModeOptions = [
  { value: 'production', label: 'Production quality' },
  { value: 'preview', label: 'Preview quick draft' },
];
